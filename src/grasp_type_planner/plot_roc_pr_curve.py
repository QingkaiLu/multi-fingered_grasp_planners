...
def plot_roc_curve(y_true, y_pred, fig_name):
    ...
    plt.plot(fpr, tpr, color='b', 
            label='Leave-one-out (AUC = %0.2f)' % roc_auc, lw=lw)
            #label='Leave one out. AUC = %0.2f.' %roc_auc, lw=lw)
    plt.plot([0, 1], [0, 1], linestyle='--', lw=lw, color='k',
                 label='Random (AUC = %0.2f).' %0.5)
    plt.xlim([-0.05, 1.05])
    plt.ylim([-0.05, 1.05])
    ...
    threshold = 0.5
    accuracy, f_score, rand_f_score = \
                compute_accuracy_f1(y_true, y_pred, threshold)
    print 'accracy, f_score, rand_f_score:'
    print accuracy, f_score, rand_f_score
...
def get_cv_roc_auc(y_true_list, y_pred_list):
    '''
    Compute the auc of ROC curve for cross validation.
    '''
    k_folds = len(y_true_list)
    roc_auc_list = []
    for i, y_true in enumerate(y_true_list):
...
def plot_average_pr_curve(y_true_list, y_pred_list, fig_name, seen_or_unseen):
    '''
    Plot average PR curve for cross validation.
    '''
    ...
    lw = 2
    plt.plot(recall, precision, color='darkorange',
                 lw=lw, label='PR curve (auc = %0.2f)' % cv_auc)
    pos_rate = np.sum(y_true) / np.shape(y_true)[0]
    plt.plot([0, 1], [pos_rate, pos_rate], color='navy', lw=lw, linestyle='--',
            label='Baseline: random guess (area = %0.2f)' % pos_rate)
    plt.xlabel('Recall')
...
def get_cv_pr_auc(y_true_list, y_pred_list):
    '''
    Compute AUC of PR curve for cross validation.
    '''
    y_true = np.concatenate(y_true_list)
    y_pred = np.concatenate(y_pred_list)
    precision, recall, _ = precision_recall_curve(y_true, y_pred)
...
def main():
    ...
    fig_name = cross_val_path + seen_or_unseen + '/avg_pr.png'
    plot_average_pr_curve(y_true_list, y_pred_list, fig_name, seen_or_unseen.title())
