...
class GenRgbdGazeboKinect:
    def __init__(self, rgbd_patches_save_path=None):
        self.rgbd_channels = 8
        self.plot_palm_fingers = False
        self.save_rgbd = False
        ...
        self.image_n_rows = 480 
        self.image_n_cols = 640
    ...
    def get_rgbd_image(self, bgr_img_path, pcd_msg, normal_msg):
        '''
        ...
        #cv2.imwrite('/home/kai/depth.png', depth_image)
        ...
        pcd_points = pc2.read_points(pcd_msg, skip_nans=False)
        pcd_points = np.array(list(pcd_points))
        print pcd_points.shape
        depth_image = pcd_points[:, 2]
        print depth_image.shape
        depth_image = np.reshape(depth_image, (self.image_n_rows, self.image_n_cols))
        depth_image[np.logical_or(np.logical_or(np.isinf(depth_image), np.isneginf(depth_image)),
                                    np.isnan(depth_image))] = 0.  


        normal_points = pc2.read_points(normal_msg, skip_nans=False)
        normal_points = np.array(list(normal_points))
        #print normal_points.shape
        normal_points = np.reshape(normal_points, (self.image_n_rows, self.image_n_cols, 4))
        ...
        # normal_points = np.flipud(normal_points)        

        rgbd_image = np.zeros((self.image_n_rows, self.image_n_cols, self.rgbd_n_channels))
