...
class GraspRgbdPatchesNet(GraspNet):
    def __init__(self):
        # Parameters
        ...
        #self.grasp_patches_file_path = rgbd_patches_save_path + 'grasps_rgbd_patches.h5'
        self.grasp_patches_file_path = rgbd_patches_save_path + 'grasp_finger_patches.h5'
    ...
    def feed_dict_func(self, train, l_rate = .0, batch_idx = None):
        ...
        if train:
            ...
            while not contain_positive_label:
                rand_indices = random.sample(range(0, self.train_samples_num), self.batch_size)
                ys = self.training_labels[rand_indices]
                contain_positive_label = np.sum(ys) > 0
    ...
    def construct_grad_graph_inf(self):
        ...
        with tf.name_scope('RGBD_GRAD'):
            ...
            #Maximize logits or self.pred?
            #The answer is to use logits. Argmax(self.pred) is the same with
            #argmax(logits), since logistic regression function is monotonically 
            #increasing. When logits is very small, logistic regression is close to
            #be zero (local minima). The graident at local minima is zero, instead of 
            #going to the local maxima, gradient descent will get stuck. That means,
            #gradient descent will not update if we start from local minimal / maximal.
            self.rgbd_gradients = self.optimizer.compute_gradients(logits[0, 0],
                    var_list=variables_list)
            #self.rgbd_gradients = self.optimizer.compute_gradients(self.pred,
            #        var_list=variables_list)
    ...
    def get_suc_prob(self, palm_patch, finger_patches):
        assign_palm_fingers_list = [self.assign_palm_var_op] + self.assign_fingers_var_ops_list
        self.sess_inf.run(assign_palm_fingers_list, feed_dict={self.holder_palm:np.array([palm_patch]), 
                    self.holder_fingers:np.array(list(map(lambda x: [x], finger_patches)))})
        
        [pred_out] = self.sess_inf.run([self.pred], feed_dict={self.keep_prob:1.})
        return pred_out[0, 0]


def main():
    grasp_rgbd_net = GraspRgbdPatchesNet()
    grasp_rgbd_net.train()
    #grasp_rgbd_net.test()
