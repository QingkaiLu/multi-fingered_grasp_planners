...
class GraspRgbdPatchesDeeperNet(GraspNet):
    def __init__(self, rgbd_patches_path=None):
        # Parameters
        ...
        self.rate_dec_epochs = 20#000
        self.training_epochs = 60#000
        self.batch_size = 4 #16
        self.display_step = 1#00
    ...
    def conv_net_hand(self, x_palm, x_fingers, w_palm, b_palm,
        ...
        with tf.name_scope('Concat'):
            fc_concat = fc_palm
            for i in xrange(self.fingers_num):
                fc_concat = tf.concat(1, [fc_concat, fc_fingers[i]])
    ...
    def create_net_var(self):
        ...
        # 11x11 conv, 8 input channels, 32 outputs
        self.conv1_w_par_palm = [11, 11, self.rgbd_channels, 96] 
        conv1_b_par_palm = [96]
        # 5x5 conv, 96 input channels, 256 outputs
        conv2_w_par_palm = [5, 5, 96, 256]
        conv2_b_par_palm = [256]
        # 3x3 conv, 256 input channels, 384 outputs
        conv3_w_par_palm = [3, 3, 256, 384]
        conv3_b_par_palm = [384]
        # 3x3 conv, 256 input channels, 384 outputs
        conv4_w_par_palm = [3, 3, 384, 384]
        conv4_b_par_palm = [384]
        # 3x3 conv, 256 input channels, 384 outputs
        conv5_w_par_palm = [3, 3, 384, 256]
        conv5_b_par_palm = [256]


        # Strides of all CNN convolution and max pool layers for the palm patch
        self.strides_palm = {
            'conv1': 4,
            'conv2': 1,
            'conv3': 1,
        ...
        # fc1, 4(conv1) * 2(max pool 1) * 2(max pool 1) * 2(max pool 1) = 32
        # 200/8 * 200/8 * 8 inputs, 32 outputs
        strides_prod_palm = self.strides_palm['conv1'] * self.strides_palm['max_pool_1'] * \
                self.strides_palm['max_pool_2'] * self.strides_palm['max_pool_3']
        fc1_inputs_num = np.ceil(float(self.palm_patch_rows) / float(self.strides_prod_palm)) * \
                np.ceil(float(self.palm_patch_cols) / float(self.strides_prod_palm))

        fc1_w_par_palm = [fc1_inputs_num, 4096]
        fc1_b_par_palm = [4096]
        fc2_w_par_palm = [4096, 4096]
        fc2_b_par_palm = [4096]
        fc3_w_par_palm = [4096, 1000]
        fc3_b_par_palm = [1000]


        # Convolution and fully connected layers weights for the palm patch
        self.weights_palm = {
        ...
        # Convolution and fully connected layers biases for the palm patch
        self.biases_palm = {
        ...
        # Max pool kernel sizes for the palm patch
        self.max_pool_k_palm = {
                'max_pool1': 3,
                'max_pool2': 3,
                'max_pool3': 3,
                }
        ...
        conv2_w_par_finger = [3, 3, 32, 8]
        conv2_b_par_finger = [8]
        # fc1, 2(conv1)*2(conv2)*2(max pool) = 8
        # ceil(100/8) * ceil(100/8) * 8 inputs, 32 outputs
        fc1_w_par_finger = [int(np.ceil(self.finger_patch_rows/8.)*np.ceil(self.finger_patch_cols/8.)*8), 32]
        ...
        self.weights_hand = {
                ...
                'fc1': tf.get_variable(name='w_fc1_hand', 
                    shape=[fc1_w_par_finger[-1] * self.fingers_num + fc1_w_par_palm[-1], 32], 
                    initializer=tf.contrib.layers.xavier_initializer_conv2d()),
    ...
    def cost_function(self):
        ...
        with tf.name_scope('F2Vis'):
            self.feature_to_vis = tf.concat(1, [fc_concat_hand, fc1_hand]) 
        ...
        with tf.name_scope('Cost'):
            ...
            self.cost = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits, self.holder_labels))
        ...
        self.train_summary = tf.merge_summary([loss_sum, pred_err_sum, learn_rate_sum])
        ...
        self.pred_err_test_sum = tf.scalar_summary('pred_err_test', self.pred_error_test)
        ...
        with tf.Session() as sess:
            ...
            for epoch in range(self.training_epochs):
                if epoch % self.rate_dec_epochs == 0 and epoch != 0:
                    learn_rate *= 0.1
                if read_data_batch and epoch % self.read_batch_epochs == 0 and epoch != 0:
                    self.read_rgbd_data_batch(self.read_data_ratio)
...
def main():
    grasp_rgbd_shallow_net = GraspRgbdPatchesDeeperNet()
    #grasp_rgbd_shallow_net.train()
    grasp_rgbd_shallow_net.test()
